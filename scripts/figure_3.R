...
comm_type_prev = read_tsv(comm_type_prev_file) %>% 
...
num_type = read_tsv(dat_file) %>% select(comm_num, comm_type) %>% unique() %>% 
...
v_prev = read_tsv(comm_v_prev_file) %>% filter(round == 18)

class_prev = read_tsv(class_prev_pred_file)
...
a1 = ggplot(nnrti_comm_type_prev, aes(x=comm_num, y=fit*100, fill=comm_type)) + 
	geom_errorbar(aes(ymin=lwr*100, ymax=upr*100), color='#333333', size=0.25, width=0) +
...
pred_vals = 
		tibble(fit_viremia=seq(0, max_val, .0001)) %>%
		mutate(
			fit_nnrti = 
...
c = ggplot(nnrti_v_comm_prev, aes(x=fit_viremia*100, y=fit_nnrti*100)) + 
		...
		geom_errorbar(aes(ymin=lwr_nnrti*100, ymax=upr_nnrti*100), color='#333333', size=0.25, width=0) +
		geom_errorbar(aes(xmin=lwr_viremia*100, xmax=upr_viremia*100), color='#333333', size=0.25, width=0) +
...
d = ggplot(nrti_v_comm_prev, aes(x=fit_viremia*100, y=fit_nrti*100)) + 
		...
		geom_errorbar(aes(ymin=lwr_nrti*100, ymax=upr_nrti*100), 
			color='#333333', size=0.25, width=0) +
...
bottom = plot_grid("",c,d,"", nrow=1,labels=c('','c','d',''), rel_widths=c(0.5,1,1,0.5))
