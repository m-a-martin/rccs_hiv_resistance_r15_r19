...
all_w = read_tsv('models/all_weights.tsv')

# todo infer this somehow?
ref_cat = c('Sex'='Female', 'Age category'='(14,24] years old', 
	'Community type' = 'Agrarian community', 'Survey round' = 'round 17')

ref_table = tibble(var_type = names(ref_cat), Variable = ref_cat, `Coefficient (95% CI)` = 'ref', 
	`p-value` = 'ref')

row_headers = tibble(var_type = names(ref_cat), Variable = names(ref_cat), 
	`Coefficient (95% CI)` = '', 
	`p-value` = '' )
...
for (i_class in c('nnrti', 'nrti', 'pi', "all")){
	o_class = read_tsv(paste(c('models/treat_', i_class, '_weights_raw.tsv'), collapse=''), show_col_types = FALSE) %>%
...
write_tsv(all_col, 'tables/table_s7.tsv')
