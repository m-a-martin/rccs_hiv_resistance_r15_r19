...
v_par[['round']] = hiv_dr_cat %>%
		...
		summarise(
			...
			n_viremic_sequenced = sum(!is.na(nrti)), 
			.group='drop') %>%
...
p_par[['round']] = hiv_dr_cat %>%
		...
		summarise(
			...
			n_pretreat_sequenced = sum(!is.na(nrti)), 
			.group='drop') %>%
		mutate(var = 'Survey round') %>%
...
t_par[['round']] = hiv_dr_cat %>%
		...
		summarise(
			...
			n_treat_sequenced = sum(!is.na(nrti)), 
			.group='drop') %>%
		mutate(var = 'Survey round') %>%
...
oa = hiv_dr_cat %>% 
	summarize(
		...
		n_viremic_sequenced = sum(viremic & (!is.na(finalhiv) & finalhiv == 'P') & (!is.na(nrti))),
		n_pretreat_participants = sum(pre_treatment & viremic & (!is.na(finalhiv) & finalhiv == 'P')),
		n_pretreat_sequenced = sum(pre_treatment & viremic & (!is.na(finalhiv) & finalhiv == 'P') & (!is.na(nrti))),
...
write_tsv(table, 'tables/table_s16.tsv')
