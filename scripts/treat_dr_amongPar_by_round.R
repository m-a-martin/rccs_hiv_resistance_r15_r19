...
for (class in c("nnrti", "nrti", "pi")){
	...
	# people living with viremic HIV who are pre-treatment: w = 1
	# people living with viremic HIV who are treatment-experienced w/ seq data: w = (1/p_i) / sum_i (1-p_i)
	# people living with viremic HIV who are treatment-experienced w/o seq data: w = 0
	class_dat = dat %>% 
		mutate(outcome_cat = case_when( 
					is.na(finalhiv) ~ 'none', # todo impute missing finalhiv measures to N 
					finalhiv == 'N' ~ 'none',
					finalhiv == 'P' & !viremic ~ 'none', 
					finalhiv == 'P' & viremic & pre_treatment == TRUE ~ 'none',
					finalhiv == 'P' & viremic & pre_treatment == FALSE & is.na(!!as.name(class)) ~ 'no_data',
					finalhiv == 'P' & viremic & pre_treatment == FALSE & !is.na(!!as.name(class)) ~ 'data'))
