...
v_par[['round']] = hiv_dr_cat %>%
		...
		summarise(
			...
			n_viremic_sequenced = sum(!is.na(nnrti)), 
			.group='drop') %>%
...
p_par[['round']] = hiv_dr_cat %>%
		...
		summarise(
			...
			n_pretreat_sequenced = sum(!is.na(nnrti)), 
			.group='drop') %>%
		mutate(var = 'Survey round') %>%
...
t_par[['round']] = hiv_dr_cat %>%
		...
		summarise(
			...
			n_treat_sequenced = sum(!is.na(nnrti)), 
			.group='drop') %>%
		mutate(var = 'Survey round') %>%
...
oa = hiv_dr_cat %>% 
	summarize(
		...
		n_viremic_sequenced = sum(viremic & (!is.na(finalhiv) & finalhiv == 'P') & (!is.na(nnrti))),
		n_pretreat_participants = sum(pre_treatment & viremic & (!is.na(finalhiv) & finalhiv == 'P')),
		n_pretreat_sequenced = sum(pre_treatment & viremic & (!is.na(finalhiv) & finalhiv == 'P') & (!is.na(nnrti))),
...
write_tsv(table, 'tables/table_s15.tsv')
