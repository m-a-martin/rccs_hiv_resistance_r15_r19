...
stopifnot(!any(is.na(pretreat_dat$outcome_cat)))
...
pretreat_dat = pretreat_dat %>% filter(w > 0)
...
for (i_mut in (pretreat_muts %>% group_by(mut) %>% summarise(n=n()) %>% arrange(-n))$mut){
	print(i_mut)
	...
	mut_dat = mut_dat %>% right_join(pretreat_dat, by=c('round', 'study_id')) %>%
		mutate(
	...
	base_output = 
		run_process_model(mut_dat, m ~ round, 
			paste(c('models/mutations/', i_mut, '_amongPretreat'), collapse=''), corstr='independence')
...
write_tsv(all_pred, 'models/dr_muts_amongPretreat_prev_pred.tsv')

all_rr = bind_rows(all_rr)
write_tsv(all_rr, 'models/dr_muts_amongPretreat_rr_pred.tsv')
