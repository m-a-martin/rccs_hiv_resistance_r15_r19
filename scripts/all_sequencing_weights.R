...
dat = hiv_dr_cat %>% filter(finalhiv == 'P' & viremic) %>%
	mutate(
	...
	filter(age_cat != '(50, 100]' & round > 16 & round < 19)
...
for (drug in c('nnrti', 'nrti', 'pi')){
		...
		write.table(o_format, 
			paste(c('models/all_', drug, '_weights_format.tsv'), collapse=''), 
			sep='\t', col.names=NA)
		options(scipen = 0)
		write.table(o, 
			paste(c('models/all_', drug, '_weights_raw.tsv'), collapse=''), 
			sep='\t', col.names=NA)
		saveRDS(m, paste(c('models/all_', drug, '_weights.rds'), collapse=''))
...
o_format[,4] = p_vals
write.table(o_format, 
	paste(c('models/all_all_weights_format.tsv'), collapse=''), 
	sep='\t', col.names=NA)
options(scipen = 0)
write.table(o, 
	paste(c('models/all_all_weights_raw.tsv'), collapse=''), 
	sep='\t', col.names=NA)
saveRDS(m, paste(c('models/all_all_weights.rds'), collapse=''))
# now predict probability of sequencing success
# among those which were successfully sequenced
p = predict(m, temp_dat, type='response')
temp_weight_dat = 
...
write_tsv(all_weights, "models/all_weights.tsv")
