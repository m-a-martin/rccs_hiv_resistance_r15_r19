...
for (drug in c('nnrti', 'nrti', 'pi')){
		...
		o <- cbind(exp(cbind(RR = r[,1], 
			       ...
			       P = r[,4])
		write.table(o, 
			paste(c('models/pretreat_', drug, '_weights_raw.tsv'), collapse=''), 
			sep='\t', col.names=NA)
		saveRDS(m, paste(c('models/pretreat_', drug, '_weights.rds'), collapse=''))
		# now predict probability of sequencing success
		# among those which were successfully sequenced for this drug
...
write_tsv(all_weights, "models/pretreat_weights.tsv")
