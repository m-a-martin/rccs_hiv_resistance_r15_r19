...
for (class in c("nnrti", "nrti", "pi")){
	...
	base_output = 
		run_process_model(class_dat, r ~ round, 
			paste(c('models/dr_', class, '_amongTreat'), collapse=''))
...
write_tsv(base_pred, 'models/dr_amongTreat_prev_pred.tsv')

stratified_pred = bind_rows(stratified_pred)
write_tsv(stratified_pred, 'models/dr_amongTreat_prev_pred_stratified.tsv')

base_rr = bind_rows(base_rr)
write_tsv(base_rr, 'models/dr_amongTreat_rr.tsv')

stratified_rr = bind_rows(stratified_rr)
write_tsv(stratified_rr, 'models/dr_amongTreat_rr_stratified.tsv')

write_tsv(bind_rows(bivar_coeffs), 'models/dr_amongTreat_bivar_coeffs.tsv')
