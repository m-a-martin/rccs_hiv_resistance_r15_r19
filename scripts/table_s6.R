...
ref_cat = c('Sex'='Female', 'Age category'='(14,24] years old', 
	'Community type' = 'Agrarian community', 'Survey round' = 'round 15')
...
for (i_class in c('all', 'nnrti', 'nrti', 'pi')){
	o_class = read_tsv(paste(c('models/pretreat_', i_class, '_weights_raw.tsv'), collapse=''), show_col_types = FALSE) %>%
...
write_tsv(all_col, 'tables/table_s6.tsv')
